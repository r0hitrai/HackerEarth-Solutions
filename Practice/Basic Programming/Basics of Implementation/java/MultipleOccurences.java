...
class MultipleOccurences {
	...
	*/
	static void merge(long[] loc, long[] arr, int left, int mid, int right) {
		int i, j, k;
		...
		long[] leftLoc = new long[leftSize];
		long[] rightLoc = new long[rightSize];
	...
	/* left is left index and right is right index of the sub-array
	   of arr to be sorted
	*/
	static void mergeSort(long[] loc, long[] arr, int left, int right) {
		if (left < right) {
	...
	public static void main(String args[]) {
		...
		int testCase, size = 0;
		long sum;
		testCase = sc.nextInt(); // setting number of test cases
		// calculating for every test case
		for (int k = 0; k < testCase; k++) {
			sum = 0;
			size = sc.nextInt(); // setting size of array
			long[] numbers = new long[size];
			long[] locations = new long[size]; // array containing all numbers and their locations
